...
### 6. AI layer with IBM WatsonX
The AI layer uses IBM WatsonX.ai to consume already-scored business events instead of replacing the streaming logic. This approach emphasizes feeding AI with fresh, processed context rather than depending on batch ETL alone.

#### WatsonX Integration
SignalTwin AI leverages IBM WatsonX.ai foundation models for generating explanations and recommendations. The integration uses:

- **IBM WatsonX.ai SDK**: Official IBM Cloud SDK for WatsonX services
- **Foundation Models**: IBM Granite models or other supported LLMs
- **Project-based Authentication**: Uses IBM Cloud API keys and project IDs
- **Configurable Model Selection**: Supports multiple WatsonX models (e.g., `ibm/granite-13b-chat-v2`)
...
- Integrate with WatsonX for recommendation explanation.
...
- Create AI recommendation payloads using WatsonX.
...
6. Show AI recommendation from WatsonX in dashboard.
...
- Build worker recommendation flow with WatsonX integration.
...
- AI recommendations from WatsonX appear in the dashboard.
...
- WatsonX integration provides enterprise-grade AI with IBM's foundation models.
...
AI agent architecture with WatsonX
FleetOps should use a small network of specialized agents powered by IBM WatsonX instead of one general agent. Suggested agents:
...
The agent uses WatsonX to generate structured recommendations.
...
Delay Agent (powered by WatsonX) recommends reroute and customer notification.

Cold Chain Agent (powered by WatsonX) recommends priority handling.
...
apps/worker → agent runner and recommendation processor with WatsonX integration.
...
Two active agents: Delay Agent and Cold Chain Agent (both using WatsonX).
...
Business value
FleetOps AI Agent can improve on-time delivery, lower spoilage, reduce dispatcher load, and create a more proactive logistics operating model. The strongest enterprise angle is that it combines live event streams, governed decision logic, and IBM WatsonX AI agent actions in one platform pattern.